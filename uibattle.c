...
static VOID
PAL_BattleUIDrawMiscMenuClassic(
   WORD       wCurrentItem,
   BOOL       fConfirmed
)
/*++
...
--*/
{
   ...
   //
   if (g_isClassicMode) {
	   PAL_BattleUIDrawMiscMenuClassic(1, TRUE);
   } else {
...
--*/
{
   ...
   if (g_isClassicMode && g_Battle.Phase == kBattlePhasePerformAction)
   {
      goto end;
   }

   if (!g_isClassicMode || !g_Battle.UI.fAutoAttack)
   {
      //
      ...
      for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
      {
		 ...
		 if (!g_isClassicMode) {
			 ...
			 else if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusParalyzed] > 0)
			 {
