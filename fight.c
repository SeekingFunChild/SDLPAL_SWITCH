...
--*/
{
   ...
   if (!g_isClassicMode) {
	   ...
	   else if (g_Battle.rgEnemy[wEnemyIndex].rgwStatus[kStatusParalyzed] != 0)
	   {
		  s *= 2;
		  s /= 3;
	   }
...
--*/
{
   ...
   if (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusHaste] != 0)
   {
	   ...
	   if (!g_isClassicMode && gpGlobals->rgPlayerStatus[wPlayerRole][kStatusParalyzed] != 0) {
		   wDexterity *= 2;
			wDexterity /= 3;
	   }
   }
...
--*/
{
   ...
   for (i = 0; i <= gpGlobals->wMaxPartyMemberIndex; i++)
   {
      ...
      else
      {
         ...
         else if (!g_isClassicMode && g_Battle.rgPlayer[i].state == kFighterAct &&
...
--*/
{
   ...
   case kBattleActionUseItem:
      if (g_isClassicMode && (gpGlobals->g.rgObject[g_Battle.rgPlayer[g_Battle.UI.wCurPlayerIndex].action.wActionID].item.wFlags & kItemFlagConsuming) == 0)
      {
         break;
      }
...
--*/
{
      ...
      g_Battle.rgEnemy[wEnemyIndex].rgwStatus[kStatusSilence] == 0)
   {
      ...
      if (gpGlobals->g.lprgMagic[wMagicNum].wType != kMagicTypeNormal)
      {
         ...
         gpGlobals->rgPlayerStatus[wPlayerRole][kStatusParalyzed] == 0 &&	// Ã»ÓÐÌ±»¾
...
//      PAL_BattleDelay(12, (WORD)(-((SHORT)wMagic)), FALSE);
   ...
   else
   {
      ...
      if (iCoverIndex == -1 &&
         (gpGlobals->rgPlayerStatus[wPlayerRole][kStatusConfused] > 0 ||
         gpGlobals->rgPlayerStatus[wPlayerRole][kStatusSleep] > 0 ||
		 gpGlobals->rgPlayerStatus[wPlayerRole][kStatusParalyzed] > 0))			// ¼´Ê±
      {
         fAutoDefend = FALSE;
      }
